...
class PHPExceller_Worksheet_ColumnCellIterator extends PHPExceller_Worksheet_CellIterator implements Iterator
{
     ...
     */
    public function __construct(PHPExceller_Worksheet $subject = null, $columnIndex = 'A', $startRow = 1, $endRow = null)
    {
        // Set subject
        $this->subject = $subject;
        $this->columnIndex = PHPExceller_Cell::columnIndexFromString($columnIndex) - 1;
    ...
    public function seek($row = 1)
    {
        if (($row < $this->startRow) || ($row > $this->endRow)) {
            throw new PHPExceller_Exception("Row $row is out of range ({$this->startRow} - {$this->endRow})");
        } elseif ($this->onlyExistingCells && !($this->subject->cellExistsByColumnAndRow($this->columnIndex, $row))) {
            throw new PHPExceller_Exception('In "IterateOnlyExistingCells" mode and Cell does not exist');
        }
        $this->position = $row;
    }
     ...
     *
     * @return PHPExceller_Worksheet_Row
     */
    ...
    public function prev()
    {
        if ($this->position <= $this->startRow) {
            throw new PHPExceller_Exception("Row is already at the beginning of range ({$this->startRow} - {$this->endRow})");
        }
    ...
    protected function adjustForExistingOnlyRange()
    {
        if ($this->onlyExistingCells) {
            ...
            if ($this->startRow > $this->endRow) {
                throw new PHPExceller_Exception('No cells exist within the specified range');
            }
