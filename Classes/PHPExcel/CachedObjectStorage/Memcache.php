...
/**
 * PHPExceller_CachedObjectStorage_Memcache
 *
 ...
 */
class PHPExceller_CachedObjectStorage_Memcache extends PHPExceller_CachedObjectStorage_CacheBase implements PHPExceller_CachedObjectStorage_ICache
{
    /**
        ...
        if ($this->currentCellIsDirty && !empty($this->currentObjectID)) {
            ...
            if (!$this->memcache->replace($this->cachePrefix . $this->currentObjectID . '.cache', $obj, null, $this->cacheTime)) {
                if (!$this->memcache->add($this->cachePrefix . $this->currentObjectID . '.cache', $obj, null, $this->cacheTime)) {
                    $this->__destruct();
                    throw new PHPExceller_Exception("Failed to store cell {$this->currentObjectID} in MemCache");
                }
            }
    ...
    public function isDataSet($pCoord)
    {
        ...
        if (parent::isDataSet($pCoord)) {
            ...
            if ($success === false) {
                ...
                throw new PHPExceller_Exception('Cell entry '.$pCoord.' no longer exists in MemCache');
            }
            return true;
        }
        return false;
        ...
        foreach ($cacheList as $cellID) {
            if ($cellID != $this->currentObjectID) {
                ...
                if ($obj === false) {
                    ...
                    throw new PHPExceller_Exception("Cell entry {$cellID} no longer exists in MemCache");
                }
                if (!$this->memcache->add($newCachePrefix . $cellID . '.cache', $obj, null, $this->cacheTime)) {
                    $this->__destruct();
                    throw new PHPExceller_Exception("Failed to store cell {$cellID} in MemCache");
                }
            }
        }
     ...
     */
    public function __construct(PHPExceller_Worksheet $parent, $arguments)
    {
        ...
        if (is_null($this->cachePrefix)) {
            ...
            if (!$this->memcache->addServer($memcacheServer, $memcachePort, false, 50, 5, 5, true, array($this, 'failureCallback'))) {
                throw new PHPExceller_Exception("Could not connect to MemCache server at {$memcacheServer}:{$memcachePort}");
            }
    ...
    {
        throw new PHPExceller_Exception("memcache {$host}:{$port} failed");
    }
