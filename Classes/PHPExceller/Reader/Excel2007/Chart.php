...
/**
 * PHPExceller_Reader_Excel2007_Chart
 *
 * @category    PHPExceller
 * @package        PHPExceller_Reader_Excel2007
 * @copyright    Copyright (c) 2021 PHPExceller
...
class PHPExceller_Reader_Excel2007_Chart
{
                        ...
                        switch ($chartDetailsKey) {
                            case "plotArea":
                                ...
                                if ($plotAreaLayout == null) {
                                    $plotAreaLayout = new PHPExceller_Chart_Layout();
                                }
                                $plotArea = new PHPExceller_Chart_PlotArea($plotAreaLayout, $plotSeries);
                            ...
                            case "legend":
                                ...
                                $legend = new PHPExceller_Chart_Legend($legendPos, $legendLayout, $legendOverlay);
                                break;
                        }
                    }
            }
        }
        $chart = new PHPExceller_Chart($chartName, $title, $legend, $plotArea, $plotVisOnly, $dispBlanksAs, $XaxisLabel, $YaxisLabel);
    ...
    private static function chartTitle($titleDetails, $namespacesChartMeta, $type)
    {
        ...
        return new PHPExceller_Chart_Title($caption, $titleLayout);
    }
...
//            echo $detailKey, ' => ',self::getAttribute($detail, 'val', 'string'),PHP_EOL;
        ...
        return new PHPExceller_Chart_Layout($layout);
    }
    ...
    private static function chartDataSeries($chartDetail, $namespacesChartMeta, $plotType)
    {
        ...
        return new PHPExceller_Chart_DataSeries($plotType, $multiSeriesType, $plotOrder, $seriesLabel, $seriesCategory, $seriesValues, $smoothLine);
    }
    ...
    private static function chartDataSeriesValueSet($seriesDetail, $namespacesChartMeta, $marker = null, $smoothLine = false)
    {
        if (isset($seriesDetail->strRef)) {
            ...
            return new PHPExceller_Chart_DataSeriesValues('String', $seriesSource, $seriesData['formatCode'], $seriesData['pointCount'], $seriesData['dataValues'], $marker, $smoothLine);
        } elseif (isset($seriesDetail->numRef)) {
            $seriesSource = (string) $seriesDetail->numRef->f;
            $seriesData = self::chartDataSeriesValues($seriesDetail->numRef->numCache->children($namespacesChartMeta['c']));

            return new PHPExceller_Chart_DataSeriesValues('Number', $seriesSource, $seriesData['formatCode'], $seriesData['pointCount'], $seriesData['dataValues'], $marker, $smoothLine);
        ...
        foreach ($titleDetailPart as $titleDetailElementKey => $titleDetailElement) {
            ...
            if (isset($titleDetailElement->rPr)) {
                ...
                if (!is_null($fontColor)) {
                    $objText->getFont()->setColor(new PHPExceller_Style_Color(self::readColor($fontColor)));
                }
                ...
                if (!is_null($underscore)) {
                    if ($underscore == 'sng') {
                        ...
                        $objText->getFont()->setUnderline(PHPExceller_Style_Font::UNDERLINE_DOUBLE);
                    } else {
                        $objText->getFont()->setUnderline(PHPExceller_Style_Font::UNDERLINE_NONE);
                    }
                }
