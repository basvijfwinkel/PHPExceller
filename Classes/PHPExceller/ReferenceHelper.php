...
class ReferenceHelper
{
     ...
     *
     * @return PHPExceller_ReferenceHelper
     */
    public static function getInstance() {
        if (!isset(self::$instance) || (self::$instance === NULL)) {
            self::$instance = new PHPExceller_ReferenceHelper();
        }
    ...
    /**
     * Create a new PHPExceller_ReferenceHelper
     */
    ...
    private static function cellAddressInDeleteRange($cellAddress, $beforeRow, $pNumRows, $beforeColumnIndex, $pNumCols) {
        list($cellColumn, $cellRow) = PHPExceller_Cell::coordinateFromString($cellAddress);
        $cellColumnIndex = PHPExceller_Cell::columnIndexFromString($cellColumn);
     ...
     *
     * @param   PHPExceller_Worksheet  $pSheet             The worksheet that we're editing
     ...
     * @param   integer             $pNumRows           Number of rows to insert/delete (negative values indicate deletion)
     */
    protected function _adjustPageBreaks(PHPExceller_Worksheet $pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows)
    {
        $aBreaks = $pSheet->getBreaks();
        ($pNumCols > 0 || $pNumRows > 0) ?
            uksort($aBreaks, array('PHPExceller_ReferenceHelper','cellReverseSort')) :
            uksort($aBreaks, array('PHPExceller_ReferenceHelper','cellSort'));
    ...
    protected function _adjustHyperlinks($pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows)
    {
        ...
        ($pNumCols > 0 || $pNumRows > 0) ?
            uksort($aHyperlinkCollection, array('PHPExceller_ReferenceHelper','cellReverseSort')) :
            uksort($aHyperlinkCollection, array('PHPExceller_ReferenceHelper','cellSort'));
    ...
    protected function _adjustDataValidations($pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows)
    {
        ...
        ($pNumCols > 0 || $pNumRows > 0) ?
            uksort($aDataValidationCollection, array('PHPExceller_ReferenceHelper','cellReverseSort')) :
            uksort($aDataValidationCollection, array('PHPExceller_ReferenceHelper','cellSort'));
    ...
    protected function _adjustProtectedCells($pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows)
    {
        ...
        ($pNumCols > 0 || $pNumRows > 0) ?
            uksort($aProtectedCells, array('PHPExceller_ReferenceHelper','cellReverseSort')) :
            uksort($aProtectedCells, array('PHPExceller_ReferenceHelper','cellSort'));
    ...
    protected function _adjustColumnDimensions($pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows)
    {
        ...
        if (!empty($aColumnDimensions)) {
            foreach ($aColumnDimensions as $objColumnDimension) {
                ...
                list($newReference) = PHPExceller_Cell::coordinateFromString($newReference);
     ...
     */
    public function insertNewBefore($pBefore = 'A1', $pNumCols = 0, $pNumRows = 0, PHPExceller_Worksheet $pSheet = NULL)
    {
        ...
        $beforeColumn    = 'A';
        $beforeRow        = 1;
        list($beforeColumn, $beforeRow) = PHPExceller_Cell::coordinateFromString($pBefore);
        $beforeColumnIndex = PHPExceller_Cell::columnIndexFromString($beforeColumn);
            ...
            for ($i = 1; $i <= $highestRow - 1; ++$i) {
                for ($j = $beforeColumnIndex - 1 + $pNumCols; $j <= $beforeColumnIndex - 2; ++$j) {
                    ...
                    if ($pSheet->cellExists($coordinate)) {
                        $pSheet->getCell($coordinate)->setValueExplicit('', PHPExceller_Cell_DataType::TYPE_NULL);
        ...
        if ($pNumRows < 0 && $beforeRow - 1 + $pNumRows > 0) {
            for ($i = $beforeColumnIndex - 1; $i <= PHPExceller_Cell::columnIndexFromString($highestColumn) - 1; ++$i) {
                for ($j = $beforeRow + $pNumRows; $j <= $beforeRow - 1; ++$j) {
                    $coordinate = PHPExceller_Cell::stringFromColumnIndex($i) . $j;
                    $pSheet->removeConditionalStyles($coordinate);
                    if ($pSheet->cellExists($coordinate)) {
                        $pSheet->getCell($coordinate)->setValueExplicit('', PHPExceller_Cell_DataType::TYPE_NULL);
                        $pSheet->getCell($coordinate)->setXfIndex(0);
                ...
                if ($pSheet->cellExists($coordinate)) {
                    ...
                    for ($j = $beforeColumnIndex - 1; $j <= $beforeColumnIndex - 2 + $pNumCols; ++$j) {
                        ...
                        if ($conditionalStyles) {
                            ...
                            $pSheet->setConditionalStyles(PHPExceller_Cell::stringFromColumnIndex($j) . $i, $cloned);
                        }
                    }
                }
        ...
        if ($pNumRows > 0 && $beforeRow - 1 > 0)
        {
            for ($i = $beforeColumnIndex - 1; $i <= PHPExceller_Cell::columnIndexFromString($highestColumn) - 1; ++$i) {
                // Style
                $coordinate = PHPExceller_Cell::stringFromColumnIndex($i) . ($beforeRow - 1);
                if ($pSheet->cellExists($coordinate)) {
                    $xfIndex = $pSheet->getCell($coordinate)->getXfIndex();
                    $conditionalStyles = $pSheet->conditionalStylesExists($coordinate) ?
                        $pSheet->getConditionalStyles($coordinate) : false;
                    for ($j = $beforeRow; $j <= $beforeRow - 1 + $pNumRows; ++$j) {
                        $pSheet->getCell(PHPExceller_Cell::stringFromColumnIndex($i) . $j)->setXfIndex($xfIndex);
                        if ($conditionalStyles) {
                            $cloned = array();
                            foreach ($conditionalStyles as $conditionalStyle) {
                                $cloned[] = clone $conditionalStyle; // TODO : GroupedConditionals need to be updated seperately
                            }
                            $pSheet->setConditionalStyles(PHPExceller_Cell::stringFromColumnIndex($i) . $j, $cloned);
                        }
                    }
                }
            }
        ...
        if (!empty($autoFilterRange)) {
            if ($pNumCols != 0) {
                ...
                if (count($autoFilterColumns) > 0) {
                    ...
                    list($rangeStart,$rangeEnd) = PHPExceller_Cell::rangeBoundaries($autoFilterRange);
                    if ($columnIndex <= $rangeEnd[0]) {
                        if ($pNumCols < 0) {
                            ...
                            for ($i = 1; $i <= $deleteCount; ++$i) {
                                if (in_array(PHPExceller_Cell::stringFromColumnIndex($deleteColumn),$autoFilterColumns)) {
                                    $autoFilter->clearColumn(PHPExceller_Cell::stringFromColumnIndex($deleteColumn));
                                }
                                ++$deleteColumn;
                            }
                        }
                        ...
                        if ($pNumCols > 0) {
                            ...
                            $startColID = PHPExceller_Cell::stringFromColumnIndex($startCol-1);
                            $toColID = PHPExceller_Cell::stringFromColumnIndex($startCol+$pNumCols-1);
                            $endColID = PHPExceller_Cell::stringFromColumnIndex($rangeEnd[0]);
                            ...
                            do {
                                $autoFilter->shiftColumn(PHPExceller_Cell::stringFromColumnIndex($endColRef-1),PHPExceller_Cell::stringFromColumnIndex($toColRef-1));
                                --$endColRef;
                                --$toColRef;
     ...
     *
     * @param PHPExceller $pPhpExcel    Object to update
     * @param string $oldName        Old name (name to replace)
     * @param string $newName        New name
     */
    public function updateNamedFormulas(PHPExceller $pPhpExcel, $oldName = '', $newName = '')
    {
        if ($oldName == '') {
            return;
        }
        ...
        foreach ($pPhpExcel->getWorksheetIterator() as $sheet) {
            foreach ($sheet->getCellCollection(false) as $cellID) {
                ...
                if (($cell !== null) && ($cell->getDataType() == PHPExceller_Cell_DataType::TYPE_FORMULA)) {
                    $formula = $cell->getValue();
                    if (strpos($formula, $oldName) !== false) {
                        $formula = str_replace("'" . $oldName . "'!", "'" . $newName . "'!", $formula);
                        $formula = str_replace($oldName . "!", $newName . "!", $formula);
                        $cell->setValueExplicit($formula, PHPExceller_Cell_DataType::TYPE_FORMULA);
                    }
                }
            }
        }
    ...
    private function updateCellRange($pCellRange = 'A1:A1', $pBefore = 'A1', $pNumCols = 0, $pNumRows = 0)
    {
        if (strpos($pCellRange, ':') !== false || strpos($pCellRange, ',') !== false) {
            // Update range
            $range = PHPExceller_Cell::splitRange($pCellRange);
            $ic = count($range);
            for ($i = 0; $i < $ic; ++$i) {
                $jc = count($range[$i]);
                for ($j = 0; $j < $jc; ++$j) {
                    if (ctype_alpha($range[$i][$j])) {
                        $r = PHPExceller_Cell::coordinateFromString($this->updateSingleCellReference($range[$i][$j].'1', $pBefore, $pNumCols, $pNumRows));
                        $range[$i][$j] = $r[0];
                    } elseif (ctype_digit($range[$i][$j])) {
                        $r = PHPExceller_Cell::coordinateFromString($this->updateSingleCellReference('A'.$range[$i][$j], $pBefore, $pNumCols, $pNumRows));
                        $range[$i][$j] = $r[1];
                    } else {
            ...
            return PHPExceller_Cell::buildRange($range);
        } else {
            throw new PHPExceller_Exception("Only cell ranges may be passed to this method.");
        }
    }
    ...
    private function updateSingleCellReference($pCellReference = 'A1', $pBefore = 'A1', $pNumCols = 0, $pNumRows = 0)
    {
        if (strpos($pCellReference, ':') === false && strpos($pCellReference, ',') === false) {
            ...
            list($newColumn, $newRow) = PHPExceller_Cell::coordinateFromString($pCellReference);

            // Verify which parts should be updated
            $updateColumn = (($newColumn{0} != '$') && ($beforeColumn{0} != '$') && (PHPExceller_Cell::columnIndexFromString($newColumn) >= PHPExceller_Cell::columnIndexFromString($beforeColumn)));
            $updateRow = (($newRow{0} != '$') && ($beforeRow{0} != '$') && $newRow >= $beforeRow);

            // Create new column reference
            if ($updateColumn) {
                $newColumn    = PHPExceller_Cell::stringFromColumnIndex(PHPExceller_Cell::columnIndexFromString($newColumn) - 1 + $pNumCols);
            }
            ...
            throw new PHPExceller_Exception("Only single cell references may be passed to this method.");
        }
    }
    ...
    {
        throw new PHPExceller_Exception("Cloning a Singleton is not allowed!");
    }
                    ...
                    foreach($matches as $match) {
                        ...
                        if ($match[3].':'.$match[4] !== $modified3.':'.$modified4) {
                            if (($match[2] == '') || (trim($match[2],"'") == $sheetName)) {
                                ...
                                $toString .= $modified3.':'.$modified4;
                                //    Max worksheet size is 1,048,576 rows by 16,384 columns in Excel 2007, so our adjustments need to be at least one digit more
                                $column = PHPExceller_Cell::columnIndexFromString(trim($match[3],'$')) + 100000;
                                $row = 10000000;
                                $cellIndex = $column.$row;
                    ...
                    foreach($matches as $match) {
                        ...
                        if ($match[3].$match[4] !== $modified3.$modified4) {
                            if (($match[2] == '') || (trim($match[2],"'") == $sheetName)) {
                                $toString = ($match[2] > '') ? $match[2].'!' : '';
                                $toString .= $modified3.':'.$modified4;
                                list($column,$row) = PHPExceller_Cell::coordinateFromString($match[3]);
                                //    Max worksheet size is 1,048,576 rows by 16,384 columns in Excel 2007, so our adjustments need to be at least one digit more
                                $column = PHPExceller_Cell::columnIndexFromString(trim($column,'$')) + 100000;
